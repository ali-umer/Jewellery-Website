...
export default function QuickView({Id, name,price,discount,handleView}: QuickViewProps) {
  const {
          quantity,
          setQuantity,
          selectedColor,
          ...
          colors,
          isLoading
        } = useProductController({ product_id: Id||2 });
...
if (isLoading) return <div>Loading product details...</div>;
  ...
  return (
    <div className="flex flex-col md:flex-row p-4 md:p-6 rounded-lg max-w-5xl mx-auto shadow-md gap-2 bg-red-900 w-[95%] md:w-auto min-h-screen md:min-h-fit relative">
      <button
        className="absolute top-4 right-4 text-white bg-black bg-opacity-30 hover:bg-opacity-50 p-2 rounded-full z-50"
      ...
      <div className="md:w-[54%] p-6 space-y-3 bg-transparent rounded-xl shadow-lg border border-amber-100 flex flex-col justify-center">
        ...
        <ColorSelector colors={colors} activeColor={selectedColor} onChange={setSelectedColor} />
